...
async def arun_one_episode(
    ...
    preplanned_turns: list[dict[str, str]] | None = None,
) -> Union[
    list[tuple[str, str, Message]],
    ...
    async def generate_messages() -> (
        ...
        while not done:
            ...
            if preplanned_turns is not None:
                # Initialize all to none
                for _idx, agent_name in enumerate(env.agents):
                    agent_messages[agent_name] = AgentAction(
                        action_type="none", argument=""
                    )
                if turn_pointer < len(preplanned_turns):
                    turn = preplanned_turns[turn_pointer]
                    # Map by agent name if possible otherwise by index position (agent1..)
                    target_agent = turn.get("agent") or f"agent{(turn_pointer % len(env.agents)) + 1}"
                    # Normalize to env agent name
                    if target_agent not in env.agents:
                        # try map index or fallback to first
                        try:
                            idx_num = int(str(target_agent).replace("agent", "")) - 1
                            target_agent = list(env.agents)[idx_num]
                        except Exception:
                            target_agent = list(env.agents)[0]
                    agent_messages[target_agent] = AgentAction(
