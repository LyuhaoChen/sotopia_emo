...
class SimulationRequest(BaseModel):
    ...
    max_turns: int
    tag: str
    # Optional preplanned turns for dataset-driven playback
    # Each item: {"agent": agent_id or name matching order, "action_type": str, "argument": str}
...
async def nonstreaming_simulation(
    ...
    # Load all agent profiles (N agents)
    agent_profiles: list[AgentProfile] = []
    for aid in simulation_request.agent_ids:
        try:
            agent_profiles.append(AgentProfile.get(pk=aid))
        except Exception:
            raise HTTPException(
                status_code=404,
                detail=f"Agent with id={aid} not found",
            )
    ...
    for idx, agent_profile in enumerate(agent_profiles):
        ...
        model_name = simulation_request.models[idx + 1]
        agent_map[agent_name] = LLMAgent(
            agent_name,
    ...
    await arun_one_episode(
        ...
        preplanned_turns=simulation_request.preplanned_turns,
    )
